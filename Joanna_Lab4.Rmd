...
tmp <- iris %>% summarise_at(vars(Sepal.Length), funs(listof_fun))
...
# this acheives the same result as line 51! (tmp and tmp1 are the same)
...
# it runs. and is more or less working. but we need to know what is the mean, sd, min, max, etc. Why is that not here? Do we need to pivot farther up in the code? I think the problem might be at the beginning with return() on line 42... 
...
# other random notes/examples, incomplete, please ignore :) 
