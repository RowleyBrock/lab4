...
is.numeric(gss_cat$tvhours)

tmp_fun(, tvhours, marital)
...
tmp_fun1 <- function(df, outcome, ..., .fun = list(n = function(x) length(x),
                                             n_valid = function(x) sum(!is.na(x)),
                                             mean = function(x) mean(x),
                                             sd = function(x) sd(x),
                                             min = function(x) min(x),
            ...
            if(is.numeric(df$outcome) == FALSE) { 
            stop("Outcome is not numeric")
            } 
...
tmp_fun(gss_cat, tvhours, age)
tmp_fun(airquality, Solar.R, Month)
